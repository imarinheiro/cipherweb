...
class DecryptForm(forms.Form):
    ...
    second_key = forms.IntegerField(label='Chave (D)', required=True, min_value=0)
    is_text = forms.BooleanField(label='Descriptografia de Texto', initial=False, required=False)
    ...
    def clean(self):
        ...
        if message and first_key:
            msg = "A chave deve ser maior para descriptografar esta mensagem"
            result = None
            if message.isdigit():
                result = int(message)
            if message.isalpha():
